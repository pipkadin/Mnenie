...
-- Справочник полов
-- ВАЖНО: используем фиксированные ID, чтобы ниже можно было ссылаться на 1/2/3
INSERT INTO POL (ID_POLA, NAIMENOVANIE) VALUES
(1, 'Мужской'),
(2, 'Женский'),
(3, 'Другое');

-- Синхронизация sequence (если далее будут вставки без явного ID)
SELECT setval('POL_ID_POLA_SEQ', (SELECT COALESCE(MAX(ID_POLA),0) FROM POL));

-- Справочник регионов
-- ВАЖНО: фиксированные ID 1..10
...
SELECT setval('REGION_ID_REGIONA_SEQ', (SELECT COALESCE(MAX(ID_REGIONA),0) FROM REGION));

-- Справочник уровней образования
-- ВАЖНО: фиксированные ID 1..4
...
SELECT setval('UROVEN_OBRAZOVANIYA_ID_UROVNYA_SEQ', (SELECT COALESCE(MAX(ID_UROVNYA),0) FROM UROVEN_OBRAZOVANIYA));
...
(1, 'Отношение к цифровизации', 'Изучение мнения населения о цифровизации', '2024-01-01', '2024-01-31', 15, 'active');
...
INSERT INTO OPROS (ID_OPROSA, TEMA, OPISANIE, DATA_NACHALA, DATA_OKONCH_PLN, MIN_KOL_OTVETOV, STATUS) VALUES
(2, 'Отношение к цифровизации', 'Изучение мнения населения о цифровизации (вторая волна)', '2024-02-01', '2024-02-28', 15, 'active');
...
INSERT INTO OPROS (ID_OPROSA, TEMA, OPISANIE, DATA_NACHALA, DATA_OKONCH_PLN, MIN_KOL_OTVETOV, STATUS) VALUES
(3, 'Экологическая сознательность', 'Изучение экологической сознательности населения', '2024-03-01', '2024-03-31', 12, 'active');
